...
def main():
    ...
    run = "2026-08-13_09-27-22_stairs"                    # None = latest exported JIT; or e.g. '2026-08-13_09-27-22_stairs'
    iteration = None              # None = highest iter for that run
    policy = None                 # explicit jit path; else logs/<exp>/<run>/policies/{run}_{iter}.pt
    plot_states = False            # False = play only, skip logging / curves / eval_results
    ...
    mujoco_model_path = os.path.join(MCJF_DIR, 'scene_stairs_L9', 'scene.xml')
    # --------------------------------------------
    ...
    class Sim2simCfg:
        ...
        class sim_config:
            mujoco_model_path = os.path.join(MCJF_DIR, 'scene_stairs_L6', 'scene.xml')
            sim_duration = 120.0
            dt = 0.005
            decimation = 4
    ...
    run_mujoco(
        policy_jit, Sim2simCfg(), commander=KeyboardCommander(vx=0.35),
